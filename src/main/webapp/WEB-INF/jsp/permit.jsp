			...
			<td>
				<table width="100%">
					<s:if test="permit.hasCompany()" >
						<tr>
							<th>Company</th> 
							<td><a href="<s:property value='#application.url' />company.action?id=<s:property value='%{permit.company.id}' />"><s:property value="%{permit.company.name}"/></a> <s:if test="permit.id == '' && permit.company.isFlagged()"><img src="<s:property value='#{application.url}' />js/images/red_flag.jpg" alt="red flagged company" width="25px"/> (<font color="red"><s:property value="permit.company.flaggedInfo" /></font>) </s:if>
							</td>
						</tr>						  
