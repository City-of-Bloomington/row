...
public class CompanyAction extends TopAction{
		...
		String contact_id="";
		static Logger logger = LogManager.getLogger(CompanyAction.class);
		//
		...
		public String execute(){
				...
				if(action.equals("Save")){
						logger.debug(" company action save");
						ret = SUCCESS;
						company.setUser_id(user.getId());
						back = company.doSave();
						if(!back.equals("")){
								addActionError(back);
								logger.error(" company action "+back);
						}
						else{
								//
								id = company.getId();
						...
						ret = SUCCESS;
						logger.debug(" company action update");
						company.setUser_id(user.getId());
						back = company.doUpdate();
						if(!back.equals("")){
								addActionError(back);
								logger.error(" company action update "+back);
						}
						else{
						...
						ret = SUCCESS;
						logger.debug(" company action remove affl");
						company.setUser_id(user.getId());
		...
		public String populate(){
				String ret = SUCCESS;
				logger.debug(" company action populate ");
				if(!id.equals("")){
